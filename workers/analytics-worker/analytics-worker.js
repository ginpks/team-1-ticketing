...
const channelName =
  process.env.PURCHASE_CONFIRMED_CHANNEL || "purchases:confirmed";
const eventBrowseQueueName =
  process.env.EVENT_BROWSE_ANALYTICS_QUEUE || "event-catalog:browsed";
const eventBrowseDlqName =
  process.env.EVENT_BROWSE_ANALYTICS_DLQ || "event-catalog:browsed:dlq";
...
const recordPurchaseConfirmed = async (purchase) => {
  const confirmedAt = purchase.confirmed_at
    ? new Date(purchase.confirmed_at)
    : new Date();
  ...
  try {
    ...
    await client.query(
      `INSERT INTO analytics (event, tickets_sold, ticket_purchase_peak_hour, revenue)
 VALUES ($1, 1, date_trunc('hour', $2::timestamp), $3)
 ON CONFLICT (event)
 DO UPDATE SET
   tickets_sold = analytics.tickets_sold + 1,
   ticket_purchase_peak_hour = date_trunc('hour', $2::timestamp),
...
const parseEventBrowseAnalytics = (rawMessage) => {
  ...
  const browsedCount = Number(
    payload.browsed_count ?? payload.browse_count ?? payload.count,
  );
  const peakHourBrowsed = new Date(
    payload.peak_hour_browsed ?? payload.peak_hour ?? payload.browsed_at,
  );
...
const pushBrowseMessageToDlq = async (rawMessage, reason) => {
  try {
    ...
    lastError = err.message;
    console.error(
      "Failed to push event browse analytics message to DLQ:",
      err.message,
    );
  }
};
...
const recordEventBrowseAnalytics = async ({
  event,
  browsedCount,
  peakHourBrowsed,
}) => {
...
const handleEventBrowseAnalytics = async (rawMessage) => {
  ...
  try {
    ...
    lastError = err.message;
    console.error(
      `Failed to update browse analytics for event ${analyticsEvent.event}:`,
      err.message,
    );
    await pushBrowseMessageToDlq(rawMessage, err.message);
  }
...
const startEventBrowseQueueConsumer = async () => {
  console.log(
    `Analytics worker listening on event browse queue ${eventBrowseQueueName}`,
  );
...
const handlePurchaseConfirmed = async (message) => {
  ...
  try {
    ...
    if (!updated) {
      console.log(
        `Skipped duplicate analytics update for purchase ${purchase.purchase_id}`,
      );
      return;
    }
    ...
    lastError = null;
    console.log(
      `Updated analytics for purchase ${purchase.purchase_id} (${purchase.event})`,
    );
  } catch (err) {
    lastError = err.message;
    console.error(
      `Failed to update analytics for purchase ${purchase.purchase_id || "unknown"}:`,
      err.message,
    );
  }
};
...
const startWorker = async () => {
  try {
    ...
    app.listen(healthPort, () => {
      console.log(
        `Analytics worker health endpoint running on port ${healthPort}`,
      );
    });
  } catch (err) {
