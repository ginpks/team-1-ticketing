...
const healthClient = redis.createClient({
  url: process.env.REDIS_URL || "redis://redis:6379",
});
...
pubClient.on("error", (err) =>
  console.error("Worker pub Redis error:", err.message),
);
healthClient.on("error", (err) =>
  console.error("Worker health Redis error:", err.message),
);
...
async function pushToDLQ(job, reason) {
  try {
    ...
    console.error(
      `Moved ${job.purchaseId || "unknown"} job to DLQ due to ${reason}`,
    );
  } catch (err) {
...
async function processJob(job) {
  try {
    ...
    try {
      ...
      console.error(
        `Payment service unreachable for purchaseId ${purchaseId}:`,
        err.message,
      );
    }
    ...
    try {
      ...
      console.log(
        `purchaseId ${purchaseId} → ${finalStatus} (${transactionRef})`,
      );
    } catch (err) {
      console.error(
        `DB update failed for purchaseId ${purchaseId}:`,
        err.message,
      );
    ...
    try {
      if (finalStatus === "confirmed") {
        ...
        console.warn(
          `Payment failed for purchaseId ${purchaseId}, adding it to the ${waitlistQueue}`,
        );
        await queueClient.rPush(waitlistQueue, JSON.stringify(job));
      }
    } catch (err) {
      console.error(
        `Pub/sub failed for purchaseId ${purchaseId}:`,
        err.message,
      );
      ...
      if (finalStatus === "confirmed") {
        try {
          ...
          console.error(
            `job with id ${job.purchaseId} failed pub/sub twice. Pushing it to the dlq.`,
          );
